`timescale 1ns / 1ps
//============================================================================
// Testbench for 2D CNN — Full-Precision + BatchNorm
//
// Architecture: Conv2D_1(1→4, 3×3) → BN1 → ReLU → Pool2D(2×2)
//               → Conv2D_2(4→8, 3×3) → BN2 → ReLU → Pool2D(2×2)
//               → FC1(200→32) → BN3 → ReLU → FC2(32→10)
//
// Loads .mem files generated by cnn2d_bn_model.py, runs inference,
// reports result. Includes BN scale/shift loading.
//============================================================================
module tb_cnn2d_bn;

    // ---- Architecture parameters (must match cnn2d_top_bn) ----
    ...
    // Timing — BN adds ~3.7K cycles: ~83K → ~87K
    parameter CLK_PERIOD_NS   = 10;
    parameter SIM_DURATION_NS = 1400000;  // 1.4 ms — generous for ~87K cycle design
    ...
    // FC biases (weights loaded internally by layer_seq_bn from .mem files)
    ...
    task display_all_outputs;
        begin
            $display("============================================================");
            $display("  2D CNN + BN OUTPUT VALUES  (Q16.16 raw logits)");
            $display("============================================================");
    ...
    initial begin
        $display("\n============================================================");
        $display("  2D CNN + BATCHNORM TESTBENCH - LOADING DATA");
        $display("============================================================\n");
        ...
        $display("[INFO] Loading Conv1 BN scale (conv1_bn_scale.mem) — %0d channels ...",
                 CONV1_OUT_CH);
        $readmemh("conv1_bn_scale.mem", conv1_bn_scale);

        $display("[INFO] Loading Conv1 BN shift (conv1_bn_shift.mem) — %0d channels ...",
                 CONV1_OUT_CH);
        $readmemh("conv1_bn_shift.mem", conv1_bn_shift);

        $display("[INFO] Loading Conv2 BN scale (conv2_bn_scale.mem) — %0d channels ...",
                 CONV2_OUT_CH);
        $readmemh("conv2_bn_scale.mem", conv2_bn_scale);

        $display("[INFO] Loading Conv2 BN shift (conv2_bn_shift.mem) — %0d channels ...",
                 CONV2_OUT_CH);
        ...
        $display("[INFO] Loading FC1 BN scale (fc1_bn_scale.mem) — %0d neurons ...",
                 FC1_OUT);
        $readmemh("fc1_bn_scale.mem", fc1_bn_scale);

        $display("[INFO] Loading FC1 BN shift (fc1_bn_shift.mem) — %0d neurons ...",
                 FC1_OUT);
        ...
        $display("\n");
        $display("############################################################");
        $display("#     2D CNN + BN INFERENCE COMPLETE - RESULTS             #");
        $display("############################################################\n");
        ...
        $display("  FC1+BN3         done at cycle: %0d", fc1_cycle);
    ...
    always @(posedge dut.pool1_done) begin
        ...
        $display("[INFO] Conv1+BN1+Pool1 DONE at %0t ns (cycle %0d).", $time, conv1_pool1_cycle);
    end
    always @(posedge dut.pool2_done) begin
        conv2_pool2_cycle = cycle_count;
        $display("[INFO] Conv2+BN2+Pool2 DONE at %0t ns (cycle %0d).", $time, conv2_pool2_cycle);
    end
    always @(posedge dut.fc1_done) begin
        if (!fc1_done_seen) begin
            fc1_done_seen = 1;
            fc1_cycle = cycle_count;
            $display("[INFO] FC1+BN3 DONE at %0t ns (cycle %0d).", $time, fc1_cycle);
        end
    end
