...
(* KEEP_HIERARCHY = "yes" *) module conv_pool_2d_cifar_ttq #(
    ...
    // 1 = mask read from BRAM, 0 = no mask (all pixels active)
    ...
    // Hysteresis activation mask — write port from mask generator
    ...
    wire skip_this_tap = skip_0_d || ($unsigned(abs_input) < $unsigned(group_min_thresh[BITS:0]));
