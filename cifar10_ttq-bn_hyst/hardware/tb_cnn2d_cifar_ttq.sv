`timescale 1ns / 1ps
//============================================================================
// Testbench — CIFAR-10 CNN (TTQ + BatchNorm + DAAP + Hysteresis)
//
// Behavioural simulation testbench for cnn2d_synth_top_cifar_ttq.
// Identical structure to baseline testbench:
//   • 40 MHz clock (25 ns period)
//   • Assert reset for 200 ns, then release
//   • Monitor per-layer done signals
//   • Monitor mask generator done signals
//   • Compare prediction against expected label
//   • Debug displays for threshold loading
//
// To run post-synthesis simulation, uncomment the line below:
...
module tb_cnn2d_cifar_ttq;
    ...
    // ================================================================
    //  DUT — TTQ + Hysteresis synth wrapper
    // ================================================================
...
`ifndef POST_SYNTHESIS
    ...
    // Mask Gen 1
    reg mask_gen1_seen = 0;
    always @(posedge clk) begin
        if (rstn && !mask_gen1_seen && u_dut.u_dut.u_mask_gen_1.done) begin
            mask_gen1_seen <= 1;
    ...
    // Mask Gen 2
    reg mask_gen2_seen = 0;
    always @(posedge clk) begin
        if (rstn && !mask_gen2_seen && u_dut.u_dut.u_mask_gen_2.done) begin
            mask_gen2_seen <= 1;
    ...
    // Mask Gen 3
    reg mask_gen3_seen = 0;
    always @(posedge clk) begin
        if (rstn && !mask_gen3_seen && u_dut.u_dut.u_mask_gen_3.done) begin
            mask_gen3_seen <= 1;
            $display("[INFO] Mask Gen 3 DONE at %0t ns (cycle %0d).", $time, cycle_count);
        end
    end
...
`ifdef POST_SYNTHESIS
    // In post-synthesis simulation, we run for a fixed number of cycles (2,900,000 cycles / 72.5 ms)
    // and then print the final predicted class and exit.
    always @(posedge clk) begin
        if (rstn && cycle_count == 2900000) begin
            $display("");
            $display("############################################################");
            $display("#  CIFAR-10 CNN + TTQ + BN + HYSTERESIS INFERENCE COMPLETE ");
            $display("############################################################");
            $display("");
...
`endif
    ...
    initial begin
        $display("");
        $display("============================================================");
        $display("  CIFAR-10 CNN + TTQ + BATCHNORM + HYSTERESIS TESTBENCH");
        ...
        $display("  Pruning:      DAAP + Spatial Hysteresis");
...
`endif
    ...
    // ================================================================
    //  Start banner after reset — includes threshold debug displays
    // ================================================================
    initial begin
        @(posedge rstn);
...
`ifndef POST_SYNTHESIS
        ...
        // Debug: verify threshold loading
        $display("[DEBUG] Conv1 act_thresh[0] = %0d (Q16.16)", u_dut.u_dut.u_conv_pool_1.act_thresh_rom[0]);
        $display("[DEBUG] Conv2 act_thresh[0] = %0d (Q16.16)", u_dut.u_dut.u_conv_pool_2.act_thresh_rom[0]);
        $display("[DEBUG] Conv3 act_thresh[0] = %0d (Q16.16)", u_dut.u_dut.u_conv_3.act_thresh_rom[0]);
        $display("[DEBUG] Conv4 act_thresh[0] = %0d (Q16.16)", u_dut.u_dut.u_conv_4.act_thresh_rom[0]);
        $display("[DEBUG] FC1 act_thresh[0]   = %0d (Q16.16)", u_dut.u_dut.u_fc1.act_thresh_rom[0]);
        $display("[DEBUG] FC2 act_thresh[0]   = %0d (Q16.16)", u_dut.u_dut.u_fc2.act_thresh_rom[0]);
        $display("");

        $display("[DEBUG] Mask1 T_H = %0d (Q16.16), T_L = %0d (Q16.16)", u_dut.u_dut.mask1_T_H, u_dut.u_dut.mask1_T_L);
        $display("[DEBUG] Mask2 T_H = %0d (Q16.16), T_L = %0d (Q16.16)", u_dut.u_dut.mask2_T_H, u_dut.u_dut.mask2_T_L);
        $display("[DEBUG] Mask3 T_H = %0d (Q16.16), T_L = %0d (Q16.16)", u_dut.u_dut.mask3_T_H, u_dut.u_dut.mask3_T_L);
        $display("");
`endif
    end
...
`endif
    ...
    always @(posedge clk) begin
        if (cycle_count >= 3500000) begin
            $display("[ERROR] Cycle count exceeded 3.5M. Terminating simulation.");
            $finish;
        end
    end
