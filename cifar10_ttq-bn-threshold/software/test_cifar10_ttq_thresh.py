...
# ===========================================================================
# CIFAR-10 TTQ+BN+Threshold — Inference / Testing + Weight Export to .mem (FPGA)
#
# This script:
#   1. Loads the trained TTQ model from cifar10_ttq_bn_thresh_model.pth
...
#   7. NEW: Exports per-filter/neuron DAAP activation threshold files
#      - conv{N}_act_thresh.mem — OUT_CH entries, Q16.16 hex
#      - fc{N}_act_thresh.mem   — NUM_NEURONS entries, Q16.16 hex
#      - Threshold formula: τ_f = best_tau_base / max(ρ_f, 0.01)
#      - Where ρ_f is the weight density of filter f
#      - If no DAAP threshold data available, export all zeros (pure TTQ mode)
# ===========================================================================
...
# ===========================================================================
# DAAP Activation Threshold utilities
# ===========================================================================
def compute_per_filter_density_conv(weight_tensor):
    """
    Compute per-filter weight density for a conv layer.
    density_f = (# non-zero ternary weights in filter f) / (total weights in filter f)
    Returns numpy array of shape [OUT_CH].
    """
    W = weight_tensor.detach().cpu()
    delta = TTQ_THRESHOLD * W.abs().max().item()
    out_ch = W.shape[0]
...
def compute_per_neuron_density_fc(weight_tensor):
    """
    Compute per-neuron weight density for an FC layer.
    density_n = (# non-zero ternary weights for neuron n) / (total weights for neuron n)
    Returns numpy array of shape [NUM_NEURONS].
    """
    W = weight_tensor.detach().cpu()
    delta = TTQ_THRESHOLD * W.abs().max().item()
    num_neurons = W.shape[0]
...
def compute_thresholds(densities, best_tau_base):
    """
    Compute per-filter/neuron DAAP activation thresholds.
    τ_f = best_tau_base / max(ρ_f, 0.01)
    Returns numpy array of thresholds (float).
    """
    clamped = np.maximum(densities, 0.01)
    thresholds = best_tau_base / clamped
    return thresholds


def export_act_thresholds(thresholds, out_filename, layer_name):
    """Export per-filter/neuron activation thresholds in Q16.16 format."""
    with open(out_filename, 'w') as f:
        f.write('\n'.join(to_fixed_point_hex(v) for v in thresholds))
    n_entries = len(thresholds)
    n_nonzero = np.count_nonzero(thresholds)
    if n_entries > 0:
        print(f"  {os.path.basename(out_filename):<28} {n_entries} entries  "
              f"range [{thresholds.min():.6f}, {thresholds.max():.6f}]  "
              f"non-zero: {n_nonzero}/{n_entries}")
    else:
        print(f"  {os.path.basename(out_filename):<28} {n_entries} entries (empty)")


def load_daap_config(script_dir):
    """
    Try to load DAAP threshold configuration.
    Looks for daap_config.txt in the script directory with format:
        best_tau_base = <float>
    Returns best_tau_base (float) or 0.0 if not found.
    """
    config_path = os.path.join(script_dir, "daap_config.txt")
    if os.path.exists(config_path):
        try:
            with open(config_path, 'r') as f:
                for line in f:
                    line = line.strip()
                    if line.startswith('best_tau_base'):
                        parts = line.split('=')
                        if len(parts) == 2:
                            val = float(parts[1].strip())
                            print(f"  [DAAP] Loaded best_tau_base = {val:.6f} from {config_path}")
                            return val
        except Exception as e:
            print(f"  [DAAP] Error reading {config_path}: {e}")
    print(f"  [DAAP] No daap_config.txt found — exporting all-zero thresholds (pure TTQ mode)")
    return 0.0
...
def main():
    print("=" * 70)
    print("  CIFAR-10 TTQ+BN+Threshold — Inference & Weight Export")
    print("=" * 70)

    SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
    MODEL_PATH = os.path.join(SCRIPT_DIR, "cifar10_ttq_bn_thresh_model.pth")
    ...
    print(f"  DAAP:   Per-filter/neuron activation thresholds (Q16.16)")
    print(f"{'='*70}")
    ...
    # ================================================================
    # 7. Export DAAP activation thresholds
    # ================================================================
    print(f"\n{'='*70}")
    print(f"  Exporting DAAP Activation Thresholds")
    print(f"{'='*70}")

    best_tau_base = load_daap_config(SCRIPT_DIR)

    # Conv thresholds
    for conv_name, conv_layer, out_ch in [
        ('conv1', model_cpu.conv1, CONV1_OUT_CH),
        ('conv2', model_cpu.conv2, CONV2_OUT_CH),
        ('conv3', model_cpu.conv3, CONV3_OUT_CH),
        ('conv4', model_cpu.conv4, CONV4_OUT_CH),
    ]:
        if best_tau_base > 0.0:
            densities = compute_per_filter_density_conv(conv_layer.weight)
            thresholds = compute_thresholds(densities, best_tau_base)
        else:
            thresholds = np.zeros(out_ch)
        export_act_thresholds(thresholds,
                              os.path.join(WEIGHTS_DIR, f"{conv_name}_act_thresh.mem"),
                              conv_name)

    # FC thresholds
    for fc_name, fc_layer, num_neurons in [
        ('fc1', model_cpu.fc1, FC1_OUT),
        ('fc2', model_cpu.fc2, FC2_OUT),
    ]:
        if best_tau_base > 0.0:
            densities = compute_per_neuron_density_fc(fc_layer.weight)
            thresholds = compute_thresholds(densities, best_tau_base)
        else:
            thresholds = np.zeros(num_neurons)
        export_act_thresholds(thresholds,
                              os.path.join(WEIGHTS_DIR, f"{fc_name}_act_thresh.mem"),
                              fc_name)
    ...
    print(f"  DAAP thresholds  : per-filter/neuron τ_f (Q16.16), τ=0 → pure TTQ")
