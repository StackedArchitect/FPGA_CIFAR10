`timescale 1ns / 1ps
//============================================================================
// Testbench — CIFAR-10 CNN (TTQ + BatchNorm + DAAP Threshold)
//
// Behavioural simulation testbench for cnn2d_synth_top_cifar_ttq.
// Identical structure to baseline testbench (tb_cnn2d_cifar_ttq.sv):
...
module tb_cnn2d_cifar_ttq;
    ...
    // ================================================================
    //  DUT — TTQ + Threshold synth wrapper
    // ================================================================
...
`ifndef POST_SYNTHESIS
    ...
    // Monitor mask generation done
    reg mask_gen1_seen = 0;
    always @(posedge clk) begin
        if (rstn && !mask_gen1_seen && u_dut.u_dut.mask_gen1_done) begin
            mask_gen1_seen <= 1;
            // Count active bits in mask1_vector
            begin
                integer m_cnt;
                m_cnt = 0;
    ...
    always @(posedge clk) begin
        if (rstn && !mask_gen2_seen && u_dut.u_dut.mask_gen2_done) begin
            mask_gen2_seen <= 1;
            begin
                integer m_cnt;
                m_cnt = 0;
    ...
    always @(posedge clk) begin
        if (rstn && !mask_gen3_seen && u_dut.u_dut.mask_gen3_done) begin
            mask_gen3_seen <= 1;
            begin
                integer m_cnt;
                m_cnt = 0;
...
`ifdef POST_SYNTHESIS
    ...
    always @(posedge clk) begin
        if (rstn && cycle_count == 2900000) begin
            $display("");
            $display("############################################################");
            $display("#  CIFAR-10 CNN + TTQ + BN + THRESHOLD INFERENCE COMPLETE    ");
            $display("############################################################");
            $display("");
...
`else
    ...
    //  Uses counter-based finish delay (NOT repeat inside always block)
    // ================================================================
...
`endif
    ...
    initial begin
        $display("");
        $display("============================================================");
        $display("  CIFAR-10 CNN + TTQ + BATCHNORM + THRESHOLD TESTBENCH");
        ...
        $display("  Threshold:    DAAP per-filter/neuron activation threshold");
...
`ifndef POST_SYNTHESIS
        ...
        $display("[DEBUG] Conv1 act_thresh[1] = %0d (Q16.16)", u_dut.u_dut.u_conv_pool_1.act_thresh_rom[1]);
