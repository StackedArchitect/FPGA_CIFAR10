`timescale 1ns / 1ps
//============================================================================
// Conv2D + BatchNorm + Optional MaxPool — CIFAR-10 TTQ (BRAM Interface)
//
// Combines:
...
//
// State flow per group per output position:
//   S_CONV_COMPUTE → S_CONV_DRAIN(×2) → S_CONV_SCALE →
//   S_CONV_BN → S_CONV_STORE → (loop store_cnt) → next position
//
// Fixed-point: Q16.16
...
(* KEEP_HIERARCHY = "yes" *) module conv_pool_2d_cifar_ttq #(
    ...
    //  PARALLEL_CH ternary accumulators active simultaneously.
    //  Shared input data, per-filter 2-bit ternary codes.
    //
    //  NO multiply in compute path — multiplier-free ternary logic.
    // ================================================================
    ...
    always @(posedge clk) begin
        if (!rstn) begin
            ...
            case (state)
                ...
                //  to prevent 32×56 DSP cascade.
                //  Applied once per store_cnt channel.
                // ==================================================
                S_CONV_SCALE: begin
